	...
	with beam.Pipeline(options=pipeline_options) as p:
		...
		class getURL(beam.DoFn):
			def process(self, element):
				...
				if lat_long:
					url_string = 'https://maps.googleapis.com/maps/api/geocode/json?latlng=%s&location_type=APPROXIMATE&result_type=country|locality&key=AIzaSyCc_Z2Jqqa1JLhZhSaURtUhF-N34PCQwrw'  % re.sub("\s+", "", lat_long[0])
